...
to setup
  ...
  ; from galaxy, at time, at distance, observed wavelength,
  ;   received-time, galaxy-distance at time received
  set Hubble-plot-data (list (list 0 0 0 0 0 0))

  set-current-plot "Spectrum from galaxy"
  ...
  [                                 ; create one Milky Way
    ...
    set xcor (-0.9) * max-pxcor + initial-distance / gly-per-pixel   ; set the turtle's initial position and heading
    ;set ycor random-ycor
    set ycor random-ycor / 10
    set heading 90
    set shape "circle"
    ...
    hatch-photons (2 * max-pycor + 1) [
      ...
      set my-id ([who] of self - ( [who] of myself + 1 ))
      ;set label my-id
    ...
    set spectrum [intensity] of photons with [generation = [generation] of myself]

    set-current-plot "Spectrum from galaxy"
    ask photons with [generation = [generation] of myself] [
      set intensity (intensity / (max spectrum))
      set color scale-color color intensity -0.1 1.5
;      plotxy wavelength intensity
    ]

    set spectrum [(list wavelength intensity)] of photons with [generation = [generation] of myself]
...
to go
  ...
  if should-stop? [
    ...
    set-current-plot "Spectrum from galaxy"
    set-current-plot-pen "received"
    set spectrum [(list wavelength intensity)] of photons with [generation = 0]
    set spectrum sort-by [ [row1 row2] -> (item 0 row1) < (item 0 row2) ] spectrum
    ;show spectrum
    set wavelengths map [ row -> item 0 row ] spectrum
    set intensities map [ row -> item 1 row ] spectrum
  ...
  [
    ...
    if xcor <= (-0.9) * max-pxcor - 1 [
      set should-stop? true
    ]
    if xcor <= (-0.9) * max-pxcor [
      ; record data to list
      set Hubble-plot-data lput (list made-by-galaxy made-at-time made-at-distance wavelength (ticks * gy-per-tick) ([separation] of turtle made-by-galaxy)) Hubble-plot-data ; from galaxy, at time, at distance, observed wavelength
;      plotxy wavelength intensity
      fd 1 - recession-rate
    ]
    if (xcor >= (max-pxcor - 1)) [
  ...
  [
    ...
    if ((ticks mod photons-every) = 0) and (xcor < (max-pxcor - 1)) and (not hidden?) and later-photons? [
      ...
      hatch-photons (2 * max-pycor + 1) [
        ...
        set made-at-time ticks
        set made-at-distance separation
        set my-id ([who] of self - ( [who] of myself + 1 + generation * (2 * max-pycor + 1)))
        ;set label my-id
        set ycor (max-pycor - my-id)
        ;set wavelength initial-wavelength + my-id * 300 / (2 * max-pycor + 1)
        set wavelength wavelength-min + my-id * (wavelength-max - wavelength-min) / (2 * max-pycor + 1)
        set color wavelength-to-color wavelength
        set intensity planck-function transparency-temperature wavelength
      ]
...
298
495
current distance, galaxy 1
[separation] of turtle 1
2
1
14
