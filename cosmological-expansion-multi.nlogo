...
to setup
  ...
  set-default-shape photons "arrow"
  set Hubble-plot-data (list (list 0 0 0 0)) ; from galaxy, at time, at distance, observed wavelength
  ...
  [                                 ; create one Milky Way
    set color red
    set xcor (-0.8) * max-pxcor + who * max-pxcor / 10    ; set the turtle's initial position and heading
...
to go
  ...
  [
    ...
    fd recession-rate
    if xcor >= (max-pxcor - 1) [stop]
    if ((ticks mod 10) = 0) and mark-positions  [
      ask patch-here [
        set pcolor 5
      ]
    ]
...
; Geoff Mathews, Foothill AstroSims
@#$#@#$#@
GRAPHICS-WINDOW
9
73
