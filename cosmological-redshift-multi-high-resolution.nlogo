...
to setup
  ...
  set-current-plot "Apparent speed of galaxies"
  ;set-current-plot-pen "distance when photon emitted"
  set-plot-pen-mode 2
...
to go
  ...
  [
    ...
    if xcor <= (-0.9) * max-pxcor [
      ...
      set-current-plot "Apparent speed of galaxies"
      ;set-current-plot-pen "distance when photon emitted"
      plotxy made-at-distance ((wavelength / initial-wavelength) ^ 2  - 1) / ((wavelength / initial-wavelength) ^ 2 + 1)
      die
    ]
    if (xcor >= (max-pxcor - 1)) [
      die
  ...
  [
    ...
    if ((ticks mod photon-every) = 0) and (xcor < (max-pxcor - 1)) and (not hidden?) [
      hatch-photons 1 [
        set label ""
        set heading 270
        set size 1
        set wavelength initial-wavelength
        set color wavelength-to-color wavelength
...
1025
654
Apparent speed of galaxies
distance when photon emitted (billion light-years)
apparent speed (% c)
0.0
20.0
0.0
...
328
704
print data to command center
print \"Photons in simulation start at 400 nm\"\nprint \"data table is:\"\nprint \"from galaxy, at time, at distance, observed wavelength, received-time, galaxy-distance at time received\"\nprint Hubble-plot-data\n
NIL
1
T
OBSERVER
...
482
498
current distance, galaxy 1 (billion light-years)
